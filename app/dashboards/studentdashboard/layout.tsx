...
export default function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    setStudentName(name);
    setStudentEmail(name || "");
    setIsVerified(true);

    // Load notifications
    if (userId) {
      fetch(`https://pariksha-9qjs.onrender.com/api/notifications/student/${userId}`)
        .then(res => res.json())
      ...
      `}>
        <div className="flex flex-col gap-6 sidebar-scrollable">
          <div className="pb-6 border-b border-white/10 mb-2">
            ...
            <p className="text-[10px] text-[#A5B4FC] font-semibold tracking-wider mt-1 uppercase m-0">Student Portal</p>
          </div>

          <nav className="flex flex-col gap-2">
            <button onClick={() => router.push("/dashboards/studentdashboard")} className={getNavClass("/dashboards/studentdashboard")}>
              <span className="text-base">📊</span> <span>Dashboard</span>
            </button>
            <button onClick={() => router.push("/dashboards/studentdashboard/view_seat_allocation")} className={getNavClass("/dashboards/studentdashboard/view_seat_allocation")}>
              <span className="text-base">💺</span> <span>My Seat Allocation</span>
            </button>
            <button onClick={() => router.push("/dashboards/studentdashboard/view_exam_routine")} className={getNavClass("/dashboards/studentdashboard/view_exam_routine")}>
        ...
        <header className="h-[70px] bg-white border-b border-[#E5E7EB] flex items-center justify-between px-4 md:px-8 sticky top-0 z-20">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-[11px] text-[#6B7280] font-medium uppercase tracking-wider m-0">Student's Portal</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            <div className="relative">
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-2 w-72 md:w-80 bg-white rounded-xl shadow-2xl border border-slate-200 max-h-96 overflow-y-auto z-50">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="hidden sm:flex flex-col text-left">
                <p className="text-[13px] font-bold text-[#111827] leading-tight m-0">{studentName}</p>
                <p className="text-[11px] text-[#6B7280] m-0 font-normal mt-0.5">{studentEmail}</p>
              </div>
            </div>
          </div>
        </header>
